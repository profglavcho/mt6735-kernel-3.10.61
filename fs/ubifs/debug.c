			       ...
			       loff_t *ppos)
{
	...
	p = ubifs_infos.next;
	pFirst = p;
	pr_err("dbg_show_all_tnc_info first p  = 0x%x,&ubifs_infos = 0x%x\n",(unsigned int)p,(unsigned int)&ubifs_infos);
	...
	while (p != &ubifs_infos) {
	    ...
	    pr_err("c->zroot.znode = 0x%x\n", (unsigned int)c->zroot.znode);

		if (c->zroot.znode)
		{
			pr_err("c->zroot.znode = 0x%x #2\n", (unsigned int)c->zroot.znode);
		
    		znode = ubifs_tnc_levelorder_next(c->zroot.znode, NULL);
    		level = znode->level;
			pr_err("c->zroot.znode = 0x%x #3\n", (unsigned int)c->zroot.znode);
		...
		{
			pr_err("c->zroot.znode = 0x%x error!!\n",(unsigned int)c->zroot.znode);
		}
		u_id++;
		...
		pr_err("dbg_show_all_tnc_info next p	= 0x%x\n",(unsigned int)p);
