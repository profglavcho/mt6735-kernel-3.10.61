...
void mtkts_allts_start_thermal_timer(void)
{
    ...
    if (thz_dev1 != NULL && interval != 0)
	    mod_delayed_work(system_freezable_wq, &(thz_dev1->poll_queue), round_jiffies(msecs_to_jiffies(60))); // 60ms
    if (thz_dev2 != NULL && interval != 0)
	    mod_delayed_work(system_freezable_wq, &(thz_dev2->poll_queue), round_jiffies(msecs_to_jiffies(60))); // 60ms
    if (thz_dev3 != NULL && interval != 0)
	    mod_delayed_work(system_freezable_wq, &(thz_dev3->poll_queue), round_jiffies(msecs_to_jiffies(60))); // 60ms
