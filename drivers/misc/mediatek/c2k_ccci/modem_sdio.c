...
*/	
    ...
    if (pure_int & SDIO_CHISR_RX_RDY) {
		...
		if((modem->msg->head.chanInfo  == SDIO_AT_CHANNEL_NUM) || (modem->msg->head.chanInfo  == DATA_CH_ID) || (modem->msg->head.chanInfo  == FLS_CH_ID) || (modem->msg->head.chanInfo == EXCP_CTRL_CH_ID)) {//
...
int sdio_modem_ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	LOGPRT(LOG_INFO,  "%s: get port done...\n",__func__);
	...
	LOGPRT(LOG_INFO,  "%s %d msg_head(0x%x, 0x%x, 0x%x, 0x%x)\n", __func__,__LINE__, msg_head->start_flag, msg_head->chanInfo, msg_head->tranHi, msg_head->tranLow);

	LOGPRT(LOG_INFO,  "%s %d msg_head addr(%p) data addr(%p)\n", __func__,__LINE__, (int *)msg_head, (int *)data);
