...
#if 1 // Turn on CPU Dormant on 32-bit
int mt_cpu_dormant_init(void);
int mt_cpu_dormant(unsigned long data);
#else // Turn off CPU Dormant on 32-bit
static int mt_cpu_dormant_init(void) { return 0; }
static int mt_cpu_dormant(unsigned long data) { return MT_CPU_DORMANT_BYPASS; }
#endif // Turn on CPU Dormant on 32-bit
