        ...
        case COMPAT_SYSRAM_ALLOC:     //
            {
                ...
                if (err)
                {
                    LOG_MSG("compat_get_isp_readirq_data error!!!\n");
                    return err;
                }
                ...
                if (err)
                {
                    LOG_MSG("compat_put_isp_readirq_data error!!!\n");
                    return err;
                }
                return ret;
            }
        case COMPAT_SYSRAM_FREE:       //
            {
                ...
                if (data == NULL)
                    return -EFAULT;
                err = compat_get_sysram_userenum_data(data32, data);
                if (err)
                {
                    LOG_MSG("compat_get_isp_readirq_data error!!!\n");
                    return err;
                }
                ret = filp->f_op->unlocked_ioctl(filp, SYSRAM_FREE,(unsigned long)data);
                err = compat_put_sysram_userenum_data(data32, data);
                if (err)
                {
                    LOG_MSG("compat_put_isp_readirq_data error!!!\n");
                    return err;
