...
static int mt_ot_gearRegister_test(struct msdc_host *host, u8 rw, u32 eco_ver, u8 *error)
{
    ...
    for(cmdrdly = 0; cmdrdly < 32; cmdrdly++)
    {
		...
		sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, cmdrdly);	// Set cmd delay

		if(cmdrdly < 32)
		{
			sdr_get_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, tmpdly);	// Get cmd delay
...
static int mt_ot_outbound_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 i_rw, u32 eco_ver, u8 *error)
{
	...
	for(i=0; i<20; i++)
	{
		sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, cmdrdly[i]);	// Set cmd delay
...
int mt_msdc_online_tuning_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 rw)
{
    ...
    sdr_get_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, default_cmdrdly);	// Get cmd delay
...
result:
	...
	sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRDLY, default_cmdrdly);	// Set default cmd delay
