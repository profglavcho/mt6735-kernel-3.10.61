        ...
        const char* pszNodeName)
{
    ...
    psGEDLogBuf->pcBuffer = ((char *)psGEDLogBuf->pMemory) + sizeof(GED_LOG_BUF_LINE) * i32MaxLineCount;
        ...
        int i32NewMaxBufferSizeByte)
{
    ...
    spin_lock_irqsave(&psGEDLogBuf->sSpinLock, psGEDLogBuf->ui32IRQFlags);
    {
        GED_LOG_BUF_LINE *pi32NewLine = (GED_LOG_BUF_LINE *)pNewMemory;
        char *pcNewBuffer = ((char *)pNewMemory) + sizeof(GED_LOG_BUF_LINE) * i32NewMaxLineCount;
    ...
    if (psGEDLogBuf)
    {
        ...
        if (psGEDLogBuf->attrs & GED_LOG_ATTR_QUEUEBUFFER)
        {
            ...
            else
            {
                ...
                /* Move buffers */
                buf_start = psGEDLogBuf->psLine[n].offset;
