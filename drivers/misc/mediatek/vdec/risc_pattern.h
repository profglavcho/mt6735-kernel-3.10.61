...
int Dump_reg( UINT32 base_r, UINT32 start_r, UINT32 end_r , char* pBitstream_name , UINT32 frame_number, BOOL bDecodeDone){
    ...
    for ( i=start_r ; i<=end_r ; i++ ){
        ...
        if ( base_r == 0xF6021000 )
            sprintf( buffer,"VLD[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6022000 )
            sprintf( buffer,"MC[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6028000 )
            sprintf( buffer,"HEVC_VLD[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6025000 )
            sprintf( buffer,"PP[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6024000 )
            sprintf( buffer,"MV[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6020000 )
            sprintf( buffer,"MISC[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
        if ( base_r == 0xF6021800 )
            sprintf( buffer,"VLD_TOP[%d] = 0x%8.0X    ",i,u4Value/*, base_r + 4*i*/);
...
void hevc_test( int frame_start , int frame_end )
{
    ...
    if (debug_mode>0)
        printk("BITSTREAM_OFFSET ioremap ptr_base = 0x%8.0X\n",(UINT)ptr_base);
    ...
    for ( i=0; i<MAX_DPB_NUM; i++ ){
        ...
        if (debug_mode>0)
            printk("mv_buf_addr[%d]:0x%8.0X ", i, mv_buf_addr[i] );
